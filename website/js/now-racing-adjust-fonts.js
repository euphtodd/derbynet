...
var FontAdjuster = {
  ...
  table_resized: function() {
    ...
    var border = this.parse_px(cells.css('border-bottom-width'));
    var available = $(window).height() - cells.position().top - border;
    var row_height = Math.floor(available / cells.length) - border;

    // 1.3 is the line height from global.css; use a larger divisor for dark theme
    // to give rows more breathing room
    var divisor = document.body.classList.contains('pintwood-dark') ? 1.5 : 1.3;
    this.original_font_size = row_height / divisor;
    $("#main-table td").css('font-size', this.original_font_size);
    this.last_table_bottom = 0;
  },
  ...
  adjust_table_fonts: function() {
    ...
    if (client_rect.bottom != this.last_table_bottom) {
      ...
      if (client_rect.bottom > $(window).height()) {
